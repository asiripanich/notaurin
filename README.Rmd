...
## Generate a keyword map from AURIN metadata
...
meta %>%
  keyword_clean(id = "aurin_open_api_id", keyword = "keywords") %>%
  pull(keyword) %>%
  make_dict -> my_dict

merged_data <- 
  meta %>%
  keyword_extract(id = "aurin_open_api_id", text = "abstract", dict = my_dict) %>%
...
grouped_data <- 
  ...
  keyword_group(com_detect_fun = group_label_prop)
